...
\subsection*{Part D: Cox Proportional Hazards Model}

To quantify the effect of stage on survival, we fit a Cox proportional hazards model using the Distant stage as the reference category.

The estimated hazard ratios are:
\begin{itemize}
    \item Localized: $\exp(-0.8625) = 0.42$
    \item Regional: $\exp(-0.8033) = 0.45$
\end{itemize}

Both Localized and Regional stages are associated with substantially lower hazards compared to the Distant stage, corresponding to approximately 55--60\% reductions in risk. These effects are highly statistically significant ($p < 0.001$).

The likelihood ratio test ($\chi^2 = 64.08$, $df = 2$, $p < 0.001$) confirms that stage at diagnosis is a strong predictor of survival.

\paragraph{Proportional hazards assumption.}
The proportional hazards assumption was assessed using Schoenfeld residuals. The test yielded a non-significant result ($p = 0.15$), indicating no evidence against the proportional hazards assumption.
\section*{Conclusion}
